...
for(run in c(8, 10)) {
  ...
  for(col in 1:4) {
    ...
    for(row in 1:50000) {
      ...
      if(difference <= -50) {
        payouts[row, col] = round(-50, 2) + plus
      } else if(difference <= 0 & difference > -50) {
        payouts[row, col] = round(difference, 2) + plus
      } else {
        earnings = round(difference * 24, 2)
        if(earnings >= 110) {
          payouts[row, col] = 110 + plus
        } else {
          payouts[row, col] = earnings + plus
        }
      }
    }
  }
...
cap = '83.67% Optimal players get a positive outcome. For betting 25%, 50% and 100%
in yellow sessions, 32.7%, 14.1% and 1.0% get a positive outcome respectively.' 
...
ggplot(aes(x = Craving, y = Payouts), data = payouts_box) +
  ...
  labs(title = 'Wager set to $10',
       caption = cap) +
  theme_minimal() +
  theme(plot.caption = element_text(hjust = 0.2),
        plot.caption.position =  "plot")

ggsave('C:/Users/samue/Google Drive/Elise Projects/Craving by Design/Plots/box plot wager 10.pdf')
